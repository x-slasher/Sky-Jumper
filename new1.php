...
function create() {
    ...
    this.game.world.setBounds(0, 0, this.game.width, 1300);

    //  A simple background for our game
    sky = game.add.tileSprite(0, 0,800,1300, 'sky');
//    sky.fixedToCamera = true;
    ...
    ledge2.bottombounds = ledge2.body.y + 120;   //move 256 pixel right
    ledge2.velo=60; 
    ...
    ledge4 = ledge4.create(game.world.centerX,game.world.height- 900, 'ledge');    
...
// adding bullet
   ...
   //  walking left and right.
    ...
    game.add.text(game.world.width - 170, game.world.height-45, 'Lives : ', { font: '24px Arial', fill: '#fff' });
    for (var i = 0; i < 3; i++) 
    {
        life = lives.create(game.world.width - 90 + (30 * i),  game.world.height+10, 'life');
   ...
   //  An explosion pool
    ...
    button = game.add.button(game.world.centerX - 80, 30, 'button', actionOnClick, this);
    butttonText = game.add.text(game.world.centerX - 60, 40,'Level-3', { fontSize: '30px', fill: '#fff' });
...
function update() {
   ...
   if(player.alive){
    ...
    if(hitLegde ||  hitLegde2 ){
        ...
        statText.position.y = player.position.y+80;
    if(hitLegde6){
     ...
     //   var random=game.rnd.integerInRange(0,3);
        ...
        if (enemyBullet )
        {
           // var shooter=villain;
            ...
            game.physics.arcade.moveToObject(enemyBullet,player,160);
